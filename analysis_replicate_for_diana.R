...
here::i_am("Equity_IPCC_AR6_pathways.Rproj")
...
save_ggplot(
  ...
  f=here("figures","diana","Table1-jsk-reproduce-v1"),
  h=100,
  w=200
)
...
save_ggplot(
  ...
  f=here("figures","diana","Table1-jsk-reproduce-v2"),
  h=100,
  w=200
)
...
save_ggplot(
  ...
  f=here("figures","diana","Table1-jsk-reproduce-v3"),
  h=100,
  w=200
)
...
p.figure2 <-  ggplot(figure2.kanitkar.data %>% pivot_wider(names_from = year, values_from = value) %>%
  ...
  labs(
    ...
    subtitle = "Each line is one scenario (only for 2050).\nBlack arrows: Kanitkar et al. (from 2020 to 2050)" ,
    caption = "2020 is only show for the Kanitkar et al. values"
  ) +
...
save_ggplot(
  p=p.figure2,
  f=here("figures","diana","Figure2-jsk-reproduce"),
  h=150,
  w=200
)
...
p.ssp.gdp <- ggplot(ssp.gdp.data.selected %>% mutate(SSP = paste0("SSP",as.character(Ssp_family))),
  ...
  labs(
    ...
    caption = "Coloured ranges: 5-95th (thin), 25-75th (thick), median (line).\nUsing variable 'GDP|PPP'."
  ) +
...
save_ggplot(
  p=p.ssp.gdp,
  f=here("figures","diana","SSP-gdp_per_capita-allc1234"),
  h=130,
  w=200
)
...
p.gini <- ggplot() +
  ...
  geom_boxplot(
    data=interregional.gini.ipcc.data %>%
      ...
      bind_rows(
        interregional.gini.ipcc.data %>%
          filter(
            year %in% c(2020,2030),
            variable=="Emissions|CO2|Energy and Industrial Processes"
          ) %>% mutate(variable = "CO2 emissions (excl. lulucf)")
      ) %>%
      filter(
        Ssp_family %in% c(1,2)
      ) %>%
...
save_ggplot(
  p=p.gini,
  f=here("figures","diana","Interregional-inequality-by-ssp"),
  h=100,
  w=150
)
